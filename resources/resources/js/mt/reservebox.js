	...
	var _main = {
			...
			del : function(schd_id_list, service_type) {
				$("#loadingLayer").show();
				var _this = this.top();
				var param = {procType:"delTranInfoList", schd_id_list:schd_id_list,reserve:"Y",service_type:service_type};
			...
			search : {
				...
				get : function(START, type) 
				{
					...
					var param = $.extend({procType:"getTranInfoList",reserve:"Y", START:START,COUNT:_this.list.vertical_count}, json);
				...
				getDate : function(){
					...
					$("#datepicker1").val(Common.currentDate());
					$("#datepicker2").val(Common.currentDate(+7));
				}
			},
			list : {
				json : "",
				...
				init : function() {
					...
					var param = $.extend( {procType:"getTranInfoList",reserve:"Y", START:"0", COUNT:_this.list.vertical_count}, json);
				...
				btn : function(){
					...
					$("td[name=subject] a",_this.list.id).unbind("click").click(function(){
						...
						var param = {schd_id:schd_id,par_date:par_date,date_client_req:date_client_req,service_type:service_type,total_cnt:total_cnt,success_cnt:success_cnt,fail_cnt:fail_cnt,wait_cnt:wait_cnt,servicetype:servicetype,content:content,file_path:file_path};
								...
								if(result_code == "1000") {			
									...
									if(data.listData.length > 0){
										for(var i=0;i<data.listData.length;i++) {
											...
											var serviceType = obj.service_type.trim();
											...
											$("td[name='subject'] a",tr).attr("servicetype", serviceType);
											$("td[name='subject'] a",tr).attr("file_path", file_path);
											...
											var nowDate_10 = nowDate + (10 * 60 * 1000);
											var strNowDate_10 = _IBTime.timestamp_to_string_full(nowDate_10);
											var send_date = date_client_req.replace(/\./gi,"-");
											if(strNowDate_10 > send_date){//취소 불가
												$("td[name='schd_id'] input",tr).prop("disabled",true);
											}
										}										
									}else{		
			...
			sendDetail : {
				...
				view : function(param){
					...
					$("#sendDetail td[name='date_client_req']").attr("service_type",param.servicetype);
					$("#sendDetail td[name='date_client_req']").attr("fail_cnt",param.fail_cnt);
					$("#sendDetail td[name='service_type']").html(param.service_type);
					...
					$("#sendDetail span[name='byte']").html(byte + "/"+limit_byte);
					$("#sendDetail div[name='mybox_image'] img").attr("src",param.file_path);
					if(param.file_path.length>0){
						$("#sendDetail div[name='mybox_image']").show();
						$("#sendDetail textarea").height("390px");
					}else{
						$("#sendDetail div[name='mybox_image']").hide();
						$("#sendDetail textarea").height("550px");
					}
				...
				resend : function(json,gubun){
					...
					//ajax : mtResendvar param = $.extend({procType:"mtsend"}, _this.send_val);
				...
				search : {
					...
					btn : function(){
						...
						$("#delBtn","#sendDetail").unbind("click").click(function(){	
							console.log('del');
							var schd_id = $("#sendDetail tbody td[name='date_client_req']").attr("schd_id");
							var service_type = $("#sendDetail tbody td[name='date_client_req']").attr("service_type");
							var date_client_req = $("#sendDetail tbody td[name='date_client_req']").html();
							var date_client_req_10 = _IBCommon.strtotime('-10 minute', date_client_req.replace(/\./gi,"-"), ".", "time").substring(0,16);
							var nowTime = _IBTime.getdatetime(0).substring(0,16);
							if(date_client_req_10 <= nowTime){//취소 불가
								alert('예약 취소는 10분전까지 가능합니다.');
							}else{
								_this.del(schd_id, service_type);
								$("#sendDetail .remodal-close").click();
					...
					get : function(START, type) 
					{
						...
						if(START == 0) _this.list.current_page = 1;
						var param = $.extend({procType:"getMtLogList",schd_id:_this.sendDetail.list.schd_id,par_date:_this.sendDetail.list.par_date,service_type:_this.sendDetail.list.service_type,START:START,COUNT:_this.list.vertical_count}, json);
									...
									if(result_code == "1000") {			
										...
										if(data.listData.length > 0){
											for(var i=0;i<data.listData.length;i++) {
												...
												var no = (_this.sendDetail.list.current_page - 1) * _this.sendDetail.list.vertical_count + i+1;
												var mt_pr = obj.mt_pr;
